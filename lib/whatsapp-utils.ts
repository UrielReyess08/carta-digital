...
export const generateWhatsAppMessage = (products: { name: string; price: number }[], total: number): string => {
  const productsList = products.map((p) => `• ${p.name} - S/.${p.price.toFixed(2)}`).join("\n")
